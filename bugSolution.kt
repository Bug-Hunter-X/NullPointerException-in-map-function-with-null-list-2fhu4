fun main() {
    ...
    val nullableResult = nullableList?.map { it * 2 } ?: emptyList() //safe call operator and elvis operator
    println(nullableResult) // Output: []
    
    val nullableList2: List<Int>? = listOf(1,2,3)
    val nullableResult2 = nullableList2?.map { it * 2 } ?: emptyList()
    println(nullableResult2) // Output: [2,4,6]
}